...
class EventPanel extends Component {
  ...
  buildEventPanel () {
    ...
    for (const index in this.props.model.notifications) {
      const event_collection = this.props.model.notifications[index]
      if (typeof event_collection !== 'undefined') {
        for (const event_obj in event_collection) {
          const note_item_id = this.uuidv4()

          const my_event = event_collection[event_obj].event
          const event_title = my_event.title
          const event_text = my_event.text
          const event_reward = my_event.reward
          const event_curse = my_event.curse
